...
SET R_VER_MINOR=6
SET R_VER_DEV=0
SET INST_PATH="%HOMEPATH%\Documents\rave_data\RAVE_install"
SET INST_TMPFILE="%HOMEPATH%\Documents\rave_data\RAVE_install\junk.txt"
SET N27_PATH="%HOMEPATH%\Documents\rave_data\others\three_brain\N27"
SET DEMO_SUB_STR="All All_that_are_not_installed KC YAB YAD YAF YAH YAI YAJ YAK"
...
if not exist %INST_PATH% mkdir %INST_PATH%


REM "Check if R is installed or the version is too low"
SET r_need_install=0

WHERE R > %INST_TMPFILE%

if %ERRORLEVEL% == 0 (
  echo Cannot find R installed anywhere
  SET r_need_install=1
) else (
  REM "Check R version"
  
  R --version | type %INST_TMPFILE%
  cat %INST_TMPFILE%
   | findstr /R [0-9]+\.[0-9]+\.[0-9]+
)

SET /p R_PATH= <  %INST_TMPFILE%
